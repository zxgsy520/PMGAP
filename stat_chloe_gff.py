...
def read_chloe_gff(file):
    ...
    for line in read_tsv(file, "\t"):
        ...
        if "gene" == line[2]:
            if attrs["ID"] in gene_dict:
                raise Exception("Gene %s has a repeat" % attrs["ID"])
            gene_dict[attrs["ID"]] = [int(line[3]), int(line[4]), line[6]]
            continue
        ...
        if line[2] not in data:
            data[line[2]] = {}
        if parent not in data[line[2]]:
            data[line[2]][parent] = []

        data[line[2]][parent].append([line[0], int(line[3]), int(line[4]), line[6]])
...
def stat_chloe_gff(file, genome, prefix="OUT"):

    gene_dict, data = read_chloe_gff(file)
...
def add_hlep_args(parser):
    ...
    parser.add_argument('gff', metavar='FILE', type=str,
        help='Input the gff file for CHLOE gene prediction.')
...
def main():
    ...
    description='''
name:
    stat_chloe_gff.py: Statistical CHLOE predicted gene structure
attention:
...
contact:  %s <%s>\
    ...
    stat_chloe_gff(args.gff, args.genome, args.prefix)
