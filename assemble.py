...
def create_unicycler_task(prefix, read1, read2, job_type, work_dir, out_dir,
                         minlen=500, thread=4):

    option = {}
    option["unicycler"] = {
        "version": get_version(SOFTWARE_VERSION["unicycler"]),
        "option": "-1 read1 -2 read2 --keep 3 --mode normal"
    }
    option["bwa"] = {
...
unicycler -1 {read1} -2 {read2} --keep 3 --mode normal -o {prefix} -t {thread}
...
python {script}/genome_stat.py --fasta {prefix}.genomic.fasta --contig {prefix}.contig_stat.tsv --base {prefix}.base_stat.tsv
...
def run_assembly(prefix, read1, read2, thread, job_type,
